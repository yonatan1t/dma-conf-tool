...
[#if DeviceTree_M33TD??]
<Project>
    <ProjectName>${projectName}_ExtMemLoader_DTS</ProjectName>
    <ProjectNature>DTS</ProjectNature>
    <DeviceTree>${DeviceTree_M33TD}</DeviceTree>
</Project>
[/#if]
[/#if]
</Workspace>
...
[#if SectionSuffix??]
<MmtSuffixSection>${SectionSuffix}</MmtSuffixSection>
[/#if]
[/#if]
[#if ota?? && ota=="true"]
<ota>true</ota>
...
[/#if]
    ...
    <Aincludes>
        ...
        <include></include>--]
    	  [#list mxIncludePaths as mxIncludePath]
                         [#if ide=="EWARM" ]
                             <include>[#if mxIncludePath ?starts_with("C:")]${mxIncludePath}[#else]$PROJ_DIR$\${mxIncludePath}[/#if]</include>
                          ...
                          [#list halIncludePaths as halIncludePath]
                               [#if ide=="EWARM" ]
                             <include>[#if halIncludePath ?starts_with("C:")]${halIncludePath}[#else]$PROJ_DIR$\${halIncludePath}[/#if]</include>
                               [#elseif ide=="MDK-ARM"  || ide=="Makefile" || ide=="CMake" || ide=="STM32CubeIDE"]
    ...
    [#if ThreadSafeSupport?? && threadsafeCore?? && ThreadSafeStrategy[threadsafeCore]??]
        <ThreadSafeSupport>
        ...
        <group>
            ...
            [#if HALGroup.sourceFilesNameList??]
                [#list HALGroup.sourceFilesNameList as filesName]
                    ...
                    [#assign removeFromConfig = "0"]
                    [#assign excludeFromConfig = "0"]
                    [#if multiConfigurationProject?? && ConfigsAndFiles?? && selectedConfig?? && ConfigsAndFiles[selectedConfig]??]
                        [#if !ConfigsAndFiles[selectedConfig]?seq_contains(filesName?replace("/","\\"))]
                            [#assign removeFromConfig = "1"]
                        [/#if]
                    [/#if]
