...
[#assign D_C_B_S_S = false][#--DUAL_CORE_BOOT_SYNC_SEQUENCE--]
...
[#assign IpInit_ToIgnore = "VREFBUF CORTEX_M4 CORTEX_M7 CORTEX_M0+ CORTEX_M33_S CORTEX_M33_NS HSEM PWR RCC CORTEX_M7_BOOT CORTEX_M7_APPLI MEMORYMAP"]
...
[/#if]
#include "${main_h}"
[#if (H7_ETH_NoLWIP?? || F4_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP?? || N6_ETH_NoLWIP?? || H5_ETH_NoTHREADX??) && HALCompliant??]
#include "string.h"
[/#if]
...
[/#if]
[#if (Secure=="false" || TZEN=="0") || (FamilyName=="STM32MP13" && FREERTOS??) || (XCUBEFREERTOS?? && FamilyName=="STM32N6") ]
...
[/#if]
[#if ((!ip?contains("LWIP")&& !ip?contains("STM32_WPAN")&& !ip?contains("wMBus")&& !ip?contains("STM32_BLE")&& !ip?contains("LoRaWAN")&& !ip?contains("SubGHz_Phy")&& !ip?contains("Sigfox") && !ip?contains("KMS") && !ip?contains("MotorControl") && (!ip?lower_case?starts_with("nucleo-") && !ip?lower_case?ends_with("-dk"))) || (ip?contains("LWIP") && (MBEDTLSUSed=="false")))]
    ...
    [#if reducevoids??]
        [#list reducevoids as voidr]
            [#if (!voidr.isNotGenerated && voidr.genCode && (voidr.ipgenericName != "SubGHz_Phy") && (((voidr.ipgenericName?lower_case)?starts_with("tim")&&(ip?lower_case)=="tim") ||(voidr.ipgenericName?lower_case==(ip?lower_case)) ||(voidr.ipgenericName?lower_case?contains("lpuart") && ip?contains("USART")))||(!voidr.isNotGenerated && voidr.genCode && (voidr.ipgenericName != "SubGHz_Phy") && voidr.ipgenericName?lower_case?contains(ip?lower_case)))]
				...
				[#if azureMW_list?contains(ip?lower_case)]					
					...
					[#if FamilyName!="STM32WBA" && FamilyName!="STM32N6"]
					  ...
					  [#elseif UsbxDeviceHost?? && UsbxDeviceHost?contains("Device")]
                        ...
                        #include "app_usbx_host.h"
					    [/#if]
					[#elseif (FamilyName=="STM32WBA" || FamilyName=="STM32N6") && ((UsbxDeviceHost?? && UsbxDeviceHost?contains("Device")) || (UsbxDeviceHost?? && UsbxDeviceHost?contains("Host")))]
                        #include "app_usbx.h"
					[/#if]
					  [#else]
                ...
                [#break]
            [/#if]
            [#if voidr.genCode && voidr.ipgenericName?lower_case == "extmem_manager" && voidr.ipgenericName?lower_case?contains(ip?lower_case)]
                #include "extmem_manager.h"
            [/#if]
        [/#list]
        [#list reducevoids as voidr]
...
/* Private typedef -----------------------------------------------------------*/
[#if HALCompliant??]
[#if (!XCUBEFREERTOS?? && (Secure=="false" || TZEN=="0")) || (FREERTOS?? && FamilyName=="STM32MP13")]
...
[#if PTAisUsed??]
[#if HALCompliant??]
#define PTA_ACTIVE_SETUP_TIME_US (30U)
[/#if]
[/#if]
/* USER CODE BEGIN PD */
[#if bootMode?? && bootMode=="boot0" && McuDualCore?? && FamilyName=="STM32H7"] [#-- boot0 sequence --]
    [#if DIE == "DIE450"]
[#assign D_C_B_S_S = true][#-- D_C_B_S_S -> DUAL_CORE_BOOT_SYNC_SEQUENCE TICKET:196328 --]
...
#endif
[#if D_C_B_S_S]#endif /* DUAL_CORE_BOOT_SYNC_SEQUENCE */[/#if]
[/#if]
...
#define VTOR_TABLE_NS_START_ADDR  0x08040000UL
[#elseif McuName?starts_with("STM32WBA5") || McuName?matches("^STM32U5...[G].*")]
...
/* USER CODE END PM */
[#if isTmpRifExist=="true" ]
[@common.optinclude name=contextFolder+mxTmpFolder+"/RIFVarsExtern.tmp"/] [#--RIF external variables --]
[/#if]
...
[#if (H7_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || F4_ETH_NoLWIP??  || H5_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP?? || N6_ETH_NoLWIP??) && HALCompliant??]
[#if H7_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP?? || N6_ETH_NoLWIP??]
#if defined ( __ICCARM__ ) /*!< IAR Compiler */
[#if (N6_ETH_NoLWIP?? && TZEN=="1" && Secure=="true")]
#pragma location=[#if RxDescAddress??]${RxDescAddress}[#else]0x34100000[/#if]
ETH_DMADescTypeDef  DMARxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_RX_CH_CNT][/#if][ETH_RX_DESC_CNT]; /* Ethernet Rx DMA Descriptors */
#pragma location=[#if TxDescAddress??]${TxDescAddress}[#else]0x341000C0[/#if]
ETH_DMADescTypeDef  DMATxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_TX_CH_CNT][/#if][ETH_TX_DESC_CNT]; /* Ethernet Tx DMA Descriptors */
[/#if]
[#if (N6_ETH_NoLWIP?? && (TZEN=="1" && Secure=="false"))]
#pragma location=[#if RxDescAddress_nonsecure??]${RxDescAddress_nonsecure}[#else]0x24100000[/#if]
ETH_DMADescTypeDef  DMARxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_RX_CH_CNT][/#if][ETH_RX_DESC_CNT]; /* Ethernet Rx DMA Descriptors */
#pragma location=[#if TxDescAddress_nonsecure??]${TxDescAddress_nonsecure}[#else]0x241000C0[/#if]
ETH_DMADescTypeDef  DMATxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_TX_CH_CNT][/#if][ETH_TX_DESC_CNT]; /* Ethernet Tx DMA Descriptors */
[/#if]
[#if H7_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP??]
...
[#if (N6_ETH_NoLWIP?? && TZEN=="1" && Secure=="true")]
__attribute__((at([#if RxDescAddress??]${RxDescAddress}[#else]0x34100000[/#if]))) ETH_DMADescTypeDef  DMARxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_RX_CH_CNT][/#if][ETH_RX_DESC_CNT]; /* Ethernet Rx DMA Descriptors */
__attribute__((at([#if TxDescAddress??]${TxDescAddress}[#else]0x341000C0[/#if]))) ETH_DMADescTypeDef  DMATxDscrTab[#if N6_ETH_NoLWIP??][ETH_DMA_TX_CH_CNT][/#if][ETH_TX_DESC_CNT]; /* Ethernet Tx DMA Descriptors */
[/#if]
[#if (N6_ETH_NoLWIP?? && (TZEN=="1" && Secure=="false"))]
__attribute__((at([#if RxDescAddress_nonsecure??]${RxDescAddress_nonsecure}[#else]0x24100000[/#if]))) ETH_DMADescTypeDef  DMARxDscrTab[#if N6_ETH_NoLWIP??][ETH_RX_DESC_CNT_nonsecure][/#if][ETH_RX_DESC_CNT_nonsecure]; /* Ethernet Rx DMA Descriptors */
__attribute__((at([#if TxDescAddress_nonsecure??]${TxDescAddress_nonsecure}[#else]0x241000C0[/#if]))) ETH_DMADescTypeDef  DMATxDscrTab[#if N6_ETH_NoLWIP??][ETH_TX_DESC_CNT_nonsecure][/#if][ETH_TX_DESC_CNT_nonsecure]; /* Ethernet Tx DMA Descriptors */
[/#if]
[#if H7_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP??]
...
[#if FamilyName=="STM32H5" || FamilyName=="STM32MP13" || FamilyName=="STM32H7RS" || FamilyName=="STM32MP2" || FamilyName=="STM32MP21"]
[#if !NETX?? || NETX =="false"]
...
[/#if]
[#if ( ( !HALCompliant?? || (isRIFUsed?? && isRIFUsed=="DMA_REQUEST") ) && isTmpRifExist=="true" )]
static void SystemIsolation_Config(void);
[/#if]
[#if HALCompliant??]
    [#list voids as void]
        [#if void.genCode && !(IpInit_ToIgnore?contains(void.ipName)) && !void.ipType?contains("thirdparty")&&!void.ipType?contains("middleware")&&!void.functionName?contains("VREFBUF")&&void.functionName!="Init" && !void.functionName?contains("MotorControl") && !void.functionName?contains("ETZPC") && !void.functionName?contains("TRACER_EMB")&&!void.functionName?contains("CORTEX_M55") && !void.functionName?contains("GUI_INTERFACE")&&!void.functionName?contains("CORTEX_A35")]
            [#if !((void.functionName?contains("RF") && FamilyName!="STM32WB")) ]
                [#if void.isStatic && !void.functionName?contains("XSPIM")&& !void.functionName?contains("USB_OTG_HS1")&& !void.functionName?contains("USB_OTG_FS1")]
				...
				[#elseif void.isStatic && void.functionName?contains("USB_OTG_FS1")]
					   static void MX_USB_OTG_FS_HCD_Init(void);
                       [#assign staticVoids =staticVoids + " "+ 'MX_USB_OTG_FS_HCD_Init']
                [#else]
...
[#if D_C_B_S_S]#if defined(DUAL_CORE_BOOT_SYNC_SEQUENCE)[/#if]
  #tint32_t timeout;
[#if D_C_B_S_S]#endif /* DUAL_CORE_BOOT_SYNC_SEQUENCE */[/#if]
  /* USER CODE END Boot_Mode_Sequence_0 */
[/#if]

[#if mpuControl?? && (FamilyName=="STM32H7" || FamilyName=="STM32H7RS" || FamilyName=="STM32F7" ||  FamilyName=="STM32N6")][#-- if MPU config is enabled --]
	[#if bootPathType??]
	[#else]
...
#tMPU_Config();
	[#else]
		[#if FamilyName !="STM32H7" && FamilyName!="STM32H7RS" && FamilyName!="STM32F7" && FamilyName!="STM32N6" ]
#n#t/* MPU Configuration--------------------------------------------------------*/
#tMPU_Config();		
		[/#if]
	[/#if]	
...
[/#list]
[#list voids as void]
[#if void.functionName?? && !(IpInit_ToIgnore?contains(void.ipName)) && !void.functionName?contains("FREERTOS")&&void.functionName!="Init" && !void.functionName?contains("Process") && !void.functionName?contains("RESMGR_UTILITY") && !void.functionName?contains("OPENAMP") && !void.functionName?contains("IPCC") && !void.functionName?contains("GTZC") && !void.functionName?contains("ETZPC") && !void.functionName?contains("TRACER_EMB") && !void.functionName?contains("GUI_INTERFACE") && !void.functionName?contains("CORTEX_A35")]
...
[#if void.functionName!="APPE_Init"]
 ...
 [#else] [#-- must generate a call in all other cases --]
  ...
  #t}
   [#else]
   [#if (!(void.functionName?contains("RF") && FamilyName!="STM32WB")) && !void.functionName?contains("CORTEX_M55")]
   [#if void.functionName?contains("USBX")]
   [#if FamilyName!="STM32U3" && FamilyName!="STM32WBA" && FamilyName!="STM32N6"]
  ...
  #tMX_EXTMEM_MANAGER_Init();
   [#else]
	[#if !void.functionName?contains("XSPIM") && !void.functionName?contains("USB_OTG_HS1") && !void.functionName?contains("USB_OTG_FS1")]
  #t${void.functionName}();
  [#elseif void.functionName?contains("USB_OTG_HS1")]
  #tMX_USB_OTG_HS_HCD_Init();
  [#elseif void.functionName?contains("USB_OTG_FS1")]
  #tMX_USB_OTG_FS_HCD_Init();
 [/#if]
   [/#if]
  [/#if]
...
#tMX_NVIC_Init();
[/#if]
[#if isTmpRifExist=="true" &&isRIFUsed?? && isRIFUsed=="DMA_REQUEST" ]
#tSystemIsolation_Config();
[/#if]
[/#compress]
...
[#if (Secure=="false" || TZEN=="0" ) || (FREERTOS?? && FamilyName=="STM32MP13") || (XCUBEFREERTOS?? && FamilyName=="STM32N6")]
...
[/#if]
[#if THREADX?? && ((FamilyName="STM32U3" && (Secure=="false" || TZEN=="0"))||(FamilyName!="STM32U3" && FamilyName!="STM32N6")||(FamilyName=="STM32N6" && THREADX_CONTEXT?? && THREADX_CONTEXT?contains(cxt)) )][#-- If ThreadX (Azure_RTOS) is used --]
[#assign isThreadxUsed=true]
[@common.optinclude name=contextFolder+mxTmpFolder+"/bsp_common_init.tmp"/]#n
  MX_ThreadX_Init();
[/#if]
[#list voids as void]
...
#n
[#-- BSP init --]
[#if !(isThreadxUsed??)] [@common.optinclude name=contextFolder+mxTmpFolder+"/bsp_common_init.tmp"/] [/#if]
...
[#if IWDG1isUsed?? && FamilyName=="STM32MP13"]
...
[/#if]
[#if ip?contains("wMBus") && !FREERTOS?? && !THREADX??]
#t#tMX_wMBus_Process();
[/#if]
[#if (S_Phy_Meter_Button?? && S_Phy_Meter_Button == "true") || (S_Phy_Meter_RadioTimer?? && S_Phy_Meter_RadioTimer == "true")]
#t#tMX_wMBus_Idle();
[/#if]
...
[/#if]
[#if instName == "USB_OTG_FS1"]
[#assign instName= "USB_OTG_FS"]
[/#if]
/**
...
[#if (DIE=="DIE4B0" || DIE =="DIE485")&& ipName?contains("USB")]
#t/* USER CODE BEGIN ${instName}_${halMode}_Init 0 */
#n
#t/* USER CODE END ${instName}_${halMode}_Init 0 */
[#elseif ipName?contains("PTA")]
#t/* USER CODE BEGIN ${instName} Config */
#n
 #t/* USER CODE END ${instName} Config */
[#else]
...
[#if (FamilyName=="STM32N6" && (instName?contains("RAMCFG")))]
[#if (SYSCFG_ITCM_size?? && SYSCFG_ITCM_size != "SYSCFG_ITCM_64K") || (SYSCFG_DTCM_size?? && SYSCFG_DTCM_size!="SYSCFG_DTCM_128K")]
uint32_t currentTCMSize = HAL_SYSCFG_GetTCMSize((uint32_t *)${SYSCFG_DTCM_size},(uint32_t *)${SYSCFG_ITCM_size});

// Check if current sizes differ from expected (MX) values
if ((currentTCMSize != SYSCFG_DTCM_128K) || (currentTCMSize != SYSCFG_ITCM_64K))
{
    /* Increase DTCM and ITCM sizes registers */
    HAL_SYSCFG_SetTCMSize(SYSCFG_DTCM_128K, SYSCFG_ITCM_64K);
...
[#if ( isTmpRifExist=="true" && ( !HALCompliant?? || (isRIFUsed?? && isRIFUsed=="DMA_REQUEST") ) )]
/**
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    HAL_IncTick();
    [#if FamilyName=="STM32U5" && USBPD_CORELIB?? && USBPD_CORELIB != "USBPDCORE_LIB_NO_PD"]
