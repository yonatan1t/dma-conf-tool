...
[#assign ram_disk_addr_s = "D1_AXISRAM_BASE"]
[#assign ram_disk_size_s = (128 * 1024)]
...
[#if SWIP.defines??]
  [#list SWIP.defines as definition]
    ...
    [/#if]    
	[#if name == "FX_SRAM_BASE_ADDRESS_S"]
      [#assign ram_disk_addr_s = value]
    [/#if]

    [#if name == "SRAM_DISK_SIZE"]
      [#assign ram_disk_size = value]
    [/#if]
	[#if name == "SRAM_DISK_SIZE_S"]
      [#assign ram_disk_size_s = value]
    [/#if]
...
[#if FamilyName=="STM32N6"]
[#if contextFolder?contains("FSBL") || contextFolder?contains("AppliSecure")]
#define FX_SRAM_DISK_BASE_ADDRESS         ${ram_disk_addr_s}
#define FX_SRAM_DISK_SIZE                 ${ram_disk_size_s}
[#else]
#define FX_SRAM_DISK_BASE_ADDRESS         ${ram_disk_addr}
#define FX_SRAM_DISK_SIZE                 ${ram_disk_size}
[/#if]
