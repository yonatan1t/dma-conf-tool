...
__IO UINT xspi_rx_cplt;
__IO UINT xspi_tx_cplt;
  [/#if]
...
[/#if]
  ...
  [#if TRANSFER_NOTIFICATION_value == "Global_state_variables"]
  /* Reset the xspi_rx_cplt to 0 before reading operation */
  xspi_rx_cplt = 0;
  [/#if]
  [#if TRANSFER_NOTIFICATION_value == "Custom"]
  /* Add any operation needed before reading operation */

  /* USER CODE BEGIN PRE_XSPI_DMA_READ */

  /* USER CODE END PRE_XSPI_DMA_READ */
...
[#if TRANSFER_NOTIFICATION_value == "Global_state_variables"]
  ...
  do
  {
    ...
    while (HAL_GetTick() - timeout_start < LX_STM32_XSPI_DEFAULT_TIMEOUT)
    {
      if (xspi_tx_cplt == 1)
      break;
    }
    ...
    else
    {
      ...
      {
        /* on Error reset the xspi_tx_cplt to 0 to make the LX_STM32_XSPI_WRITE_CPLT_NOTIFY() fail */
        xspi_tx_cplt = 0;
        return 1;
      }
      else
...
/* at this stage the write operation is successful and the xspi_tx_cplt is set to 1
 * thus LX_STM32_XSPI_WRITE_CPLT_NOTIFY() will correctly notify the low-level driver
 */
 [/#if]
 [#if TRANSFER_NOTIFICATION_value == "Custom"]
   /* Calculation of the size between the write address and the end of the page */
