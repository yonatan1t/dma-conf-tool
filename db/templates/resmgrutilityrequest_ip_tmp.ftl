...
[#macro generateResMgrCode(RESMGR_ID, valueResMgrRequest )]
    #n
    #tif (ResMgr_Request(RESMGR_RESOURCE_RIFSC,RESMGR_RIFSC_${valueResMgrRequest}_ID) != RESMGR_STATUS_ACCESS_OK)
    #t{
...
[#list IPdatas as IP]
    [#list IP.configModelList as configModel]
        [#list configModel.refConfigFiles as refConfigFile]
            [#if refConfigFile.fileName == "resmgrutilityrequest_" + resMgrRequest + ".tmp"]
                [#assign valueCore = ""]
