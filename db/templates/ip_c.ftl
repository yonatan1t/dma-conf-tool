...
[#if name=="ETH" && (F4_ETH_NoLWIP?? || F7_ETH_NoLWIP??  || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP??|| N6_ETH_NoLWIP?? || H5_ETH_NoTHREADX?? )]
#include "string.h"
[/#if]
...
[#if name=="ETH" && (H7_ETH_NoLWIP?? || F7_ETH_NoLWIP?? || H5_ETH_NoLWIP?? || MP13_ETH_NoLWIP?? || H7RS_ETH_NoLWIP?? || MP2_ETH_NoLWIP?? || MP21_ETH_NoLWIP??|| N6_ETH_NoLWIP??)]
[#if !H5_ETH_NoLWIP??]
...
[/#list]
    [#if serviceType=="Init"]
        [#if !ipName?contains("I2C")&& !ipName?contains("USB") && !ipName?contains("MRSUBG_TIMER")] [#-- if not I2C --]
                ...
                [#if initService.clock!="none"]
                  [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
                    ...
                    [#list initService.clock?split(';') as clock]
                        ...
                        [#elseif FamilyName=="STM32WL3" && (ipName=="MRSUBG" ||ipName=="LPAWUR")]
    ...
    #t#t${clock?replace("ENABLE","DISABLE")?trim}[#if !clock?contains('(')]()[/#if];
         ...
         [#else]
            ...
            [#elseif FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
			  [#if ipName !="MRSUBG_TIMER"]
                 #t#t/* Peripheral clock disable */
...
[#-- if I2C clk_enable should be after GPIO Init Begin --]
	...
	#t#t}
	[/#if]
	[#if ipName?contains("USB") && ((FamilyName=="STM32H5" && DIE != "DIE474") || FamilyName=="STM32N6")]
	#n#t#t/* Enable VDDUSB */
	#t#tHAL_PWREx_EnableVddUSB();
	[/#if]
           [#if initService.clock??]
            [#if initService.clock!="none"]
              [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
               #t#t/* ${ipName} clock enable */
                [#list initService.clock?split(';') as clock]
                    [#if ipvar.clkCommonResource.entrySet()?contains(clock?trim) && DIE != "DIE4B0" && (DIE != "DIE485" || (DIE == "DIE485" && ipvar.ipName != "USB_OTG")) ]#t#t${clock?trim?replace("__","")?replace("_ENABLE","")}_ENABLED++;
                    #t#tif(${clock?trim?replace("__","")?replace("_ENABLE","")}_ENABLED==1){
                        #t#t#t${clock?trim}();
                    [#if ipvar.clkCommonResource.entrySet()?contains(clock?trim)]#t#t}[/#if]
                    [#else]
...
#n
[#if (instName.contains("USB_OTG_HS")) && ( DIE == "DIE476" ||  DIE == "DIE481")]
#t#t__HAL_RCC_SYSCFG_CLK_ENABLE();
[/#if]
...
[#if instName?starts_with("GPDMA") | instName?starts_with("LPDMA")| (instanceData.usedDriver?? && instanceData.usedDriver!="HAL")][#--Check if LL driver is used. instanceData:ConfigModel --]
    ...
    [#if instanceData.initServices?? && instanceData.initServices.gpio?? ]
        [#assign service=instanceData.initServices.gpio]
           ...
           [/#list]
    [/#if]
    [#if  !(instName?starts_with("GPDMA")) && !(instName?starts_with("HPDMA")) && !(instName?starts_with("LPDMA")) && instanceData.initServices?? && instanceData.initServices.dma?? && (FamilyName=="STM32U5" | FamilyName=="STM32H5" | FamilyName=="STM32H7RS"| FamilyName=="STM32U3"| FamilyName=="STM32WBA"| FamilyName=="STM32N6" | FamilyName=="STM32MP2")]
...
[#if ((name == "MRSUBG" || name == "MRSUBG_TIMER" || name == "LPAWUR")  && FamilyName=="STM32WL3") || name == "PTA"]
   ...
   [#else]
        ...
        [#if  word0.contains("ADF")||word0.contains("MDF")]              
          [#if DIE == "DIE485" || DIE == "DIE454"]
            #tif[#if word0.contains("ADF")](IS_MDF_ALL_INSTANCE(${mode?lower_case}Handle->Instance))[/#if][#if word0.contains("MDF")](IS_MDF_INSTANCE(${mode?lower_case}Handle->Instance))[/#if]
          [#else]
        ...
        [#else]
			...
			[#if  !(word0.contains("MRSUBG")||word0.contains("LPAWUR") || word0.contains("PTA"))]
...
[#else]
    [#if words[0]??]
[#if words[0] == "MRSUBG"]
#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 0 */

#n#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 0 */
[#else]
#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspInit 0 */
...
[/#list]
    ...
    [@generateServiceCode ipName=words[0] serviceType="Init" modeName=mode instHandler=mode?lower_case+"Handle" tabN=2/]
[#if words[0] == "MRSUBG"]
#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 1 */

#n#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 1 */
[#else]
#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspInit 1 */
...
[#if !words[0].contains("MRSUBG") && !words[0].contains("LPAWUR") && !words[0].contains("PTA")]
#t}
[/#if]
[/#if]
[/#if]
...
[#if words[0]??]
[#if words[0] == "MRSUBG"]
#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 0 */

#n#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 0 */
[#else]
#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspDeInit 0 */
...
[/#if]
[#if words[0] == "MRSUBG"]
#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 1 */

#n#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 1 */
[#else]
#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspDeInit 1 */
