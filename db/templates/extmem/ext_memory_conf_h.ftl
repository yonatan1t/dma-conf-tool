...
[#assign RefParam_XIP_ApplicationOffsetNS_value = "not defined"]
...
[#assign EXTMEM_LRUN_TZ_ENABLE_NS_value = "valueNotSetted"]
[#assign EXTMEM_LRUN_DESTINATION_ADDRESS_NS_value = "valueNotSetted"]
[#assign EXTMEM_LRUN_SOURCE_ADDRESS_NS_value = "valueNotSetted"]
...
[#if SWIP.defines??]
  [#list SWIP.defines as definition]
    ...
    [/#if]
	[#if Name == "RefParam_XIP_ApplicationOffsetNS"]
      [#assign RefParam_XIP_ApplicationOffsetNS_value = Value]
    [/#if]
	...
	[#-- 213378 --]
    [#if Name == "EXTMEM_LRUN_TZ_ENABLE_NS"]
      [#assign EXTMEM_LRUN_TZ_ENABLE_NS_value = Value]
    [/#if]
	[#if Name == "EXTMEM_LRUN_DESTINATION_ADDRESS_NS"]
      [#assign EXTMEM_LRUN_DESTINATION_ADDRESS_NS_value = Value]
    [/#if]
	[#if Name == "EXTMEM_LRUN_SOURCE_ADDRESS_NS"]
      [#assign EXTMEM_LRUN_SOURCE_ADDRESS_NS_value = Value]
    [/#if]
...
[#if RefParam_BOOT_enable_value == "true"]
    [#if RefParam_BOOT_selection_value == "XIP" && (FamilyName != "STM32N6" && FamilyName != "STM32H7RS" || contextFolder != "ExtMemLoader/")]
#include "boot/stm32_boot_xip.h"
    [/#if]
    [#if RefParam_BOOT_selection_value == "LRUN" && (FamilyName != "STM32N6" && FamilyName != "STM32H7RS" || contextFolder != "ExtMemLoader/")]
...
#define EXTMEM_XIP_IMAGE_OFFSET ${RefParam_XIP_ApplicationOffset_value}
   [/#if]
   [#if RefParam_XIP_ApplicationOffsetNS_value!="not defined" && RefParam_XIP_ApplicationOffsetNS_value!="valueNotSetted"]
#define EXTMEM_XIP_IMAGE_OFFSET_NS ${RefParam_XIP_ApplicationOffsetNS_value}
   [/#if]
[/#if]
...
#define EXTMEM_LRUN_SOURCE EXTMEMORY_2
  [/#if]
  [#if RefParam_LRUN_SRC_offset_value!="valueNotSetted"]
#define EXTMEM_LRUN_SOURCE_ADDRESS  ${RefParam_LRUN_SRC_offset_value}u
[/#if]
  [#if RefParam_LRUN_SRC_size_value!="valueNotSetted"]
...
#define EXTMEM_LRUN_DESTINATION_INTERNAL  1
  [/#if]
  [#if RefParam_LRUN_DEST_offset_value!="valueNotSetted"]
#define EXTMEM_LRUN_DESTINATION_ADDRESS ${RefParam_LRUN_DEST_offset_value}u
 [/#if]
 [/#if]

 [#if EXTMEM_LRUN_TZ_ENABLE_NS_value !="valueNotSetted"]
   [#if EXTMEM_LRUN_TZ_ENABLE_NS_value == "1"]
#define EXTMEM_LRUN_TZ_ENABLE_NS 
   [#else]
/* #define EXTMEM_LRUN_TZ_ENABLE_NS */
   [/#if]
 [/#if]
 [#if EXTMEM_LRUN_DESTINATION_ADDRESS_NS_value !="valueNotSetted"]
#define EXTMEM_LRUN_DESTINATION_ADDRESS_NS ${EXTMEM_LRUN_DESTINATION_ADDRESS_NS_value}u
 [/#if]
 [#if EXTMEM_LRUN_SOURCE_ADDRESS_NS_value !="valueNotSetted"]
#define EXTMEM_LRUN_SOURCE_ADDRESS_NS ${EXTMEM_LRUN_SOURCE_ADDRESS_NS_value}u
 [/#if]
[/#if]
