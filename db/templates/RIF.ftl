...
[#if isRIFUsed??&& isRIFUsed=="DMA_REQUEST_LL"]
[#-- No think to do --]
[#else]
 ...
 [#assign oldValue=""]
    [#list RIMU as rimu ]
        ...
        [#if rimu.secure == "true" || rimu.privilege == "true"]
             [#assign preFixS= (rimu.secure == "true")?then("","N") ]
             [#assign preFixP= (rimu.privilege == "true")?then("","N") ]
             [#assign ipName=Rename(RIMU_IP_RENAME,rimu.ipName)]

            [#if lock == "false"]
                #n#t/*RIMC configuration*/
                #tRIMC_MasterConfig_t RIMC_master = {0};
                #tRIMC_master.MasterCID = RIF_CID_${rimu.cid};
                #tRIMC_master.SecPriv = RIF_ATTRIBUTE_${preFixS}SEC | RIF_ATTRIBUTE_${preFixP}PRIV;
                [#assign lock = "true"]
                [#assign oldValue=rimu]
            [/#if]

             [#if rimu.cid != oldValue.cid]
                  #tRIMC_master.MasterCID = RIF_CID_${rimu.cid};
             [/#if]
             [#if rimu.secure != oldValue.secure || rimu.privilege != oldValue.privilege]
                  #tRIMC_master.SecPriv = RIF_ATTRIBUTE_${preFixS}SEC | RIF_ATTRIBUTE_${preFixP}PRIV;
             [/#if]
             #tHAL_RIF_RIMC_ConfigMasterAttributes(RIF_MASTER_INDEX_${ipName}, &RIMC_master);#n
    ...
    [#list RISAF_BASE_REGEION as baseRegion ]
        [#list baseRegion.entrySet() as entry]
            [#if lock == "false"]
                ...
                [#if baseRegion.keySet()?contains("NPU")]
                #n#t/* Enable NPU Clock */
                #t__HAL_RCC_NPU_CLK_ENABLE();
                #t__HAL_RCC_NPU_FORCE_RESET();
                #t__HAL_RCC_NPU_RELEASE_RESET();
                #n
                #t__HAL_RCC_AXISRAM4_MEM_CLK_ENABLE();
                #tCLEAR_BIT(RAMCFG_SRAM4_AXI->CR, RAMCFG_CR_SRAMSD);
                #t__HAL_RCC_AXISRAM5_MEM_CLK_ENABLE();
                #tCLEAR_BIT(RAMCFG_SRAM5_AXI->CR, RAMCFG_CR_SRAMSD);
                #t__HAL_RCC_AXISRAM6_MEM_CLK_ENABLE();
                #tCLEAR_BIT(RAMCFG_SRAM6_AXI->CR, RAMCFG_CR_SRAMSD);
                [/#if]
                [#assign lock = "true"]
            [/#if]
        ...
        [#list ip.entrySet() as entry]
            ...
            [#if ipName?matches("HPDMA1|GPDMA1")]
                ...
                [#list ipData  as value]
                    ...
                    #t/* set ${value.Feature} used by ${value.IP_Request} */
                    [#if value.isHal=="true"]
                         #tif (HAL_DMA_ConfigChannelAttributes(&handle_${Channel[0]}_Channel${Channel[2]},${getRefernce("DMA_SecPriv",value.Secure+","+value.Privilege,"")}|${getRefernce("DMA_SrcDest",value.SRC_SEC+","+value.DEST_SEC,"")})!= HAL_OK )
                        #t{
                        #t#tError_Handler();
                        #t}
                    [#elseif value.isHal=="false" ]
                        #tLL_DMA_${(value.Secure=="true")?then("Enable", "Disable")}ChannelSecure(${Channel[0]}, LL_DMA_CHANNEL_${Channel[2]});
                        #tLL_DMA_${(value.Privilege=="true")?then("Enable", "Disable")}ChannelPrivilege(${Channel[0]}, LL_DMA_CHANNEL_${Channel[2]});
                        #tLL_DMA_${(value.SRC_SEC=="true")?then("Enable", "Disable")}ChannelSrcSecure(${Channel[0]}, LL_DMA_CHANNEL_${Channel[2]});
                        #tLL_DMA_${(value.DEST_SEC=="true")?then("Enable", "Disable")}ChannelDestSecure(${Channel[0]}, LL_DMA_CHANNEL_${Channel[2]});
                    [#else]
            ...
            [#if ipName?matches("RTC|TAMP") && RTC_Flag]
                ...
                [#if ip.get("RTC")?? ]
                    [#list ip.get("RTC")  as value]
                        [#if value.IP_Context?? && value.IP_Context?contains(Context)]
...
[#function getRefernce key value value2]
    ...
    [#elseif key == "DMA_SrcDest"]
        [#assign ret=value?split(",")]
        [#return (ret[0]=="true")?then("DMA_CHANNEL_SRC_SEC", "DMA_CHANNEL_SRC_NSEC")+"|"+(ret[1]=="true")?then("DMA_CHANNEL_DEST_SEC", "DMA_CHANNEL_DEST_NSEC")]
    [#else]
        [#return value]
    [/#if]
[/#function]
...
[#function getEndAddress startAddress ReginSize]
        [#assign  intStartAddress =Long.parseUnsignedLong( startAddress?substring(2), 16)]
        [#assign  intReginSize =Long.parseUnsignedLong(ReginSize?substring(2), 16)]
