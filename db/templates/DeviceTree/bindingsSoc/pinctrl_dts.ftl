...
[#compress]
		...
		[#if ip?has_content]
			[#if nodeName?has_content][#--mandatory--]
				...
				[#--Commented line: replaced by MatchFwr column in the pinout file --]
				[#--if !((deviceFwName != "OP-TEE") && deviceName?? && (deviceName == "rcc")) || mx_socDtRPN?starts_with("stm32mp2")]--]
						[#assign generateNode = true]
				[#--/#if--]
				[/#if]
			[#else]
...
[/#compress]
	 [#if srvcmx_isTargetedFw_inDTS("EL")]
			[#local fwsNameEL = ""]
		 	[#local ipsAssignedExmemLoader=[]]
			[#local fwsNameEL = functionTemplateDts(mxDtDM.dts_template)]
			[#list ipsAssignedToExmemLoaderFws.entrySet() as ipsAssignedExmemLoaderFws]
				[#if (ipsAssignedExmemLoaderFws.key)?? && ipsAssignedExmemLoaderFws.key == fwsNameEL]
					[#local ipsAssignedExmemLoader = ipsAssignedExmemLoaderFws.value]
		 		[/#if]
			[/#list]
		[/#if]
		 	[#if nodeName?? && nodeName?has_content]
			 		[#local ipName = (nodeName?matches(".*[0-9]$"))?then(nodeName?substring(0,nodeName?length-1),nodeName)]
 						[#if ipName?starts_with("USB")]
							[#local ipName ="USB"]
						[/#if]
						[#if ipName?starts_with("ospi")]
							[#local ipName ="octospim"]
						[/#if]
						[#if ipName?starts_with("eth")]
							[#local ipName ="eth"]
						[/#if]
				 [#if srvcmx_isTargetedFw_inDTS("EL") && (!ipsAssignedExmemLoader?contains(ipName))]
					 [#continue]
 				[/#if]
 				[/#if]
...
[#compress]
				...
				[#list dtPinCtrlDataModel.get(nodeRule).entrySet() as gpioparamEntry][#--For all signals of the IP--]
					...
					[#assign gpioConfig= gpioConfig+"&"+"st,io-delay = <"+dlyValue+">"]
					[/#if]
					[#if dtkey??]
