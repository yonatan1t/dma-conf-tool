...
[#t]
	...
	[#if flexgenNb > 0]
		...
		[#if srvcmx_isTargetedFw_inDTS("MCUBOOT")]
			[#list peripheralRCCParams.get("CLKFLEXGENM3").entrySet() as paramEntry]
				[#if paramEntry.key == "XBAR${FlexgenNb-1}CLKSourceARGM3"][#assign XBARCLKSourceARGM3="${paramEntry.value}"]
					[#if XBARCLKSourceARGM3??]
						[#lt]${T2}FLEXGEN_CFG(${FlexgenNb-1}, ${XBARCLKSourceARGM3}, ${XBARPrediv-1}, ${XBARFindiv-1})
					[/#if]
				[/#if]
			[/#list]
		[/#if]
		[#if srvcmx_isTargetedFw_inDTS("OP-TEE")]
			...
			[/#list]
		[/#if]
		[#if srvcmx_isTargetedFw_inDTS("TF-M") || srvcmx_isTargetedFw_inDTS("EL")]
			[#list peripheralRCCParams.get("CLKFLEXGENTF").entrySet() as paramEntry]
				[#if paramEntry.key == "XBAR${FlexgenNb-1}CLKSourceARGTF"][#assign XBARCLKSourceARGTF="${paramEntry.value}"]
					[#if XBARCLKSourceARGTF??]
						[#lt]${T2}FLEXGEN_CFG(${FlexgenNb-1}, ${XBARCLKSourceARGTF}, ${XBARPrediv-1}, ${XBARFindiv-1})
					[/#if]
				[/#if]
			[/#list]
		[/#if]
...
[#list peripheralRCCParams.get("CLKFLEXGENTF").entrySet() as paramEntry]
	[#if paramEntry.key == "ContextM33TD"][#assign ContextM33TD=paramEntry.value?number]

[#t]
	[#if ((srvcmx_isTargetedFw_inDTS("TF-M")||srvcmx_isTargetedFw_inDTS("MCUBOOT")) && ContextM33TD == 1) || (ContextM33TD == 0)]
	[#lt]${T1}st,busclk = <
[#t]
...
[#if srvcmx_isTargetedFw_inDTS("TF-M")]
	[#list peripheralRCCParams.get("CLKSystemSourceTF").entrySet() as paramEntry]
		[#lt]${T2}MUX_CFG(${paramEntry.value})
	[/#list]
    [#list peripheralRCCParams.get("CLKDTPLLSource").entrySet() as paramEntry]
		[#if paramEntry.key == "PLL1SourceARG"][#assign PLL1Source=paramEntry.value]
			[#lt]${T2}MUX_CFG(${PLL1Source})
		[/#if]
	[/#list]
[/#if]
[#if srvcmx_isTargetedFw_inDTS("MCUBOOT")]
	[#list peripheralRCCParams.get("CLKSystemSourceBOOT").entrySet() as paramEntry]
		[#lt]${T2}MUX_CFG(${paramEntry.value})
	[/#list]
[/#if]
[#if srvcmx_isTargetedFw_inDTS("OP-TEE")]
...
[#t]
	[#list peripheralParams.get("RCC").entrySet() as paramEntry]
		...
		[#if paramEntry.key == "PLL${PLLnb}UsedBOOT"][#assign PLLUsedBOOT=paramEntry.value?number][/#if]
		[#if paramEntry.key == "PLL${PLLnb}UsedTFM"][#assign PLLUsedTFM=paramEntry.value?number][/#if]
	...
	[/#list]
    [#if ((PLLUsedTFA! = 1) && srvcmx_isTargetedFw_inDTS("TF-A")) || ((PLLUsedA7! = 1) && srvcmx_isTargetedFw_inDTS("OP-TEE")) || ((PLLUsedTFM! = 1) && srvcmx_isTargetedFw_inDTS("TF-M")) || ((PLLUsedBOOT! = 1) && srvcmx_isTargetedFw_inDTS("MCUBOOT"))]
		[#if PLLSource!= "0"]
		[#lt]${T1}pll${PLLnb}:st,pll-${PLLnb?number} {
			[#if mx_tdcidContext?starts_with("A35") || (mx_tdcidContext?starts_with("M33") && !srvcmx_isTargetedFw_inDTS("MCUBOOT") && !srvcmx_isTargetedFw_inDTS("TF-M"))][#lt]${T2}st,pll = <&pll${PLLnb}_cfg_${FOUTPOSTDIV}MHz>;[/#if]
