...
#include "dt-bindings/clock/${mx_socDtRPN}-clksrc.h"
    [#if mx_tdcidContext?matches("A[0-9][0-9].*")]
#include "dt-bindings/reset/${mx_socDtRPN}-resets.h"
    [/#if]
#include <dt-bindings/pinctrl/stm32-pinfunc.h>
    [#if mx_tdcidContext?matches("M[0-9][0-9].*")]
#include <dt-bindings/rif/stm32mp25-rif.h>
#include <dt-bindings/rif/${mx_socDtRPN}-rifsc.h>
#include <dt-bindings/rif/${mx_family}-risaf.h>
...
[#if mx_tdcidContext?matches("M[0-9][0-9].*")]
[#--hpdma instances to generate in tf-m for m33td--]
[@DTBindedDtsElmtDMsList_print  pParentElmt="" pElmtsList=srvcmx_getElmtsMatchingBindedHwName(mxDtDM.dts_bindedElmtsList, "hpdma_m33td") pDtLevel=0 pOrdering=true/]
[/#if]
#n
