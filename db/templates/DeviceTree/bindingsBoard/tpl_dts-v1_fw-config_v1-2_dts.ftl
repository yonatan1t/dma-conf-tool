			...
			[#if (RIF_Params.key)?? && RIF_Params.key == "risaf4"]
				...
				[#list BKPSRAMTable.entrySet() as RISAFTable_params]
					[#if (RISAFTable_params.key == "7" || RISAFTable_params.key == "8" )]
						...
						[#if (RISAFTable_params.value[UiRegionName_Index])?matches("^\\{.+=.+\\}$")][#-- to discard empty region names --]
						    	[#if (EncryptMap[EncryptKey])=="true" || (EncryptMap[EncryptKey])=="Fast Block Cipher"]
                            		[#local EncryptList += ["RIF_ENC_EN"]]
                            	[#elseif (EncryptMap[EncryptKey])=="Block Cipher" ]
                            		[#local EncryptList += ["RIF_ENC_MCE_EN"]] [#--case MP21 only--]
                            	[#else]
                            		[#local EncryptList += ["RIF_ENC_DIS"]]
                                [/#if]
						[/#if]
						[/#list]
