...
[#macro srvcmx_init]
	...
	[#list configs as mxMcuDataModel]
		...
		[#assign mx_peripheralGPIOParams= mxMcuDataModel.peripheralGPIOParams]
		[#if mxMcuDataModel.contextTDCID??]
			[#assign mx_tdcidContext= mxMcuDataModel.contextTDCID]
		[/#if]
		[#if mxMcuDataModel.ahbErrata??]
			[#assign mx_isAhbErrata= mxMcuDataModel.ahbErrata]
		[/#if]
		...
		[#assign RisupMcuBootMap = mxMcuDataModel.risupMcuBootMap]
		[#--MCU Contexts info --]
        ...
        [#--IPCC info --]
        [#assign mx_IpccEnabledModes = mxMcuDataModel.enabledIpccModes]

		[#assign ipsAssignedToExmemLoaderFws = mxMcuDataModel.ipsAssignedToExmemLoaderFws]
		[#assign RisupMcuBootMap = mxMcuDataModel.risupMcuBootMap]
		[#assign rifawareMcubootFeatures = mxMcuDataModel.rifawareMcubootFeatures]
...
[#--------------------------------------------------------------------------------------------------------------------------------]
[#-- Ip Assignement Exmem Loader --]
[#--------------------------------------------------------------------------------------------------------------------------------]
[#function functionTemplateDts  temp]
	[#assign fwsName = ""]
        [#if temp?contains("op-tee")]
        [#assign fwsName = "OP-TEE"]
        [#elseif  temp?contains("tf-a")]
       		 [#assign fwsName = "TF-A"]
		[#else]
			[#assign fwsName = "U-BOOT"]
		[/#if]
		[#return fwsName]
[/#function]

[#function srvcmx_isContextMcuBoot   keyIn]
[#local keysList = []]	
	[#list RisupMcuBootMap.entrySet() as entry]
		[#local keysList = keysList + [entry.key]]
	[/#list]
 [#if keysList?seq_contains(keyIn) && srvcmx_isTargetedFw_inDTS("MCUBOOT")]
 [#return true]
	[#else]
		[#return false]
[/#if]
...
[#function srvcmx_getListIpMcuboot]
[#local keysList = []]	
	[#list RisupMcuBootMap.entrySet() as entry]
		[#local keysList = keysList + [entry.key]]
	[/#list]
...
[#function srvcmx_getListFeaturesGPIO ]
	[#local featureGpioList = []]
	[#list rifawareMcubootFeatures.entrySet() as elmt]
		[#if elmt.key?matches("GPIO")]
			[#local featureGpioList = featureGpioList + [elmt.value]]
		[/#if]	
	[/#list]
	[#return featureGpioList]
[/#function]


[#function srvcmx_getListFeaturesName ]
	[#local elmtsList = []]
	[#list rifawareMcubootFeatures.entrySet() as elmt]
	[#if !elmt.key?matches("GPIO")]	
		[#local elmtsList = elmtsList + [elmt.value]]
	[/#if]	
	[/#list]
