	...
	[#--FIX: optimize when pElmtsList is empty--]
		...
		[/#if]
			...
			[#if elmt.typeName?? && elmt.typeName?has_content]
			 	[#if elmt.deviceName?? && elmt.deviceName?has_content]
			 		[#local ipName = (elmt.deviceName?matches(".*[0-9]$"))?then(elmt.deviceName?substring(0,elmt.deviceName?length-1),elmt.deviceName)]
 						[#if ipName?starts_with("usb")]
							[#local ipName ="usb"]
						[/#if]
						[#if ipName?starts_with("ommanager")]
							[#local ipName ="octospim"]
						[/#if]
				[#if srvcmx_isTargetedFw_inDTS("EL") && ipsAssignedExmemLoader??]
   					 [#local willcontinue=false]
    				 [#list  ipsAssignedExmemLoader as SingleIPExtmemloader]
        				[#if (SingleIPExtmemloader?matches(ipName+".*"))]
            				[#local willcontinue=true]
        				[/#if]
   					 [/#list]
    				[#if !willcontinue]
       					 [#continue]
