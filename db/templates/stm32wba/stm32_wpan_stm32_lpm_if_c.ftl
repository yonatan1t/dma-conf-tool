...
[#list  MappedPins as item]
  ...
  [/#list]
  [#if x>0]
    [#if !item[0]?contains("RCC") && !item[0]?contains("I2C") && !item[0]?contains("DEBUG") && !item[0]?contains("SAI")]
  /* ${item[0]} */
    [/#if]
  [/#if]
  [#assign x = 0]
  ...
  LL_GPIO_SetPinMode(GPIOB, LL_GPIO_PIN_0, LL_GPIO_MODE_OUTPUT);
    ...
    [#case "USART3"]
  /* TX */
      [#if item[1]?contains("PA7")]
  LL_GPIO_SetOutputPin(GPIOA, LL_GPIO_PIN_7);
  LL_GPIO_SetPinMode(GPIOA, LL_GPIO_PIN_7, LL_GPIO_MODE_OUTPUT);
      [/#if]
      [#if item[1]?contains("PB10")]
  ...
  LL_GPIO_SetPinMode(GPIOH, LL_GPIO_PIN_3, LL_GPIO_MODE_OUTPUT);
    ...
    [#case "SPI1"]
      [#assign mosi_port = "X"]
      [#assign miso_port = "X"]
      [#if item[1]?contains("PA15")]
        [#assign mosi_port = "A"]
      [/#if]
      [#if item[1]?contains("PB3")]
        [#assign miso_port = "B"]
      [/#if]
  if (READ_BIT(SPI1->CFG2, SPI_CFG2_MASTER) != 0UL)
  {
    /* SCK */
    if (LL_GPIO_IsInputPinSet(GPIOB, LL_GPIO_PIN_4) == 1UL)
    {
  ...
  else
  {
    ...
    [#case "SPI2"]
      ...
      [#if item[1]?contains("PB0")]
        ...
        [#assign mosi_pin = "0"]
      [/#if]
      [#if item[1]?contains("PA9")]
        [#if sck_port?contains("X")]
          [#assign sck_port = "A"]
          [#assign sck_pin = "9"]
        [#else]
          [#assign miso_port = "A"]
  ...
  {
    /* SCK */
    if (LL_GPIO_IsInputPinSet(GPIO${sck_port}, LL_GPIO_PIN_${sck_pin}) == 1UL)
    {
      LL_GPIO_SetOutputPin(GPIO${sck_port}, LL_GPIO_PIN_${sck_pin});
    }
    else
    {
      LL_GPIO_ResetOutputPin(GPIO${sck_port}, LL_GPIO_PIN_${sck_pin});
    }
    LL_GPIO_SetPinMode(GPIO${sck_port}, LL_GPIO_PIN_${sck_pin}, LL_GPIO_MODE_OUTPUT);
      [#if !mosi_port?contains("X")]
    /* MOSI */
    LL_GPIO_SetPinMode(GPIO${mosi_port}, LL_GPIO_PIN_${mosi_pin}, LL_GPIO_MODE_OUTPUT);
      [/#if]
  }
      [#if !miso_port?contains("X")]
  else
  {
    /* MISO */
    LL_GPIO_SetPinMode(GPIO${miso_port}, LL_GPIO_PIN_${miso_pin}, LL_GPIO_MODE_OUTPUT);
  }
      [/#if]
      [#break]
    [#case "SPI3"]
      ...
      [#assign miso_port = "X"]
      [#assign mosi_port = "X"]
      [#if item[1]?contains("PA8")]
        [#assign sck_port = "A"]
        [#assign sck_pin = "8"]
      [/#if]
      [#if item[1]?contains("PA0")]
        [#assign sck_port = "A"]
        [#assign sck_pin = "0"]
      [/#if]
      [#if item[1]?contains("PA1")]
        [#assign miso_port = "A"]
        [#assign miso_pin = "1"]
      [/#if]
      [#if item[1]?contains("PB9")]
        [#assign miso_port = "B"]
        [#assign miso_pin = "9"]
      [/#if]
      [#if item[1]?contains("PC4")]
        ...
        [#assign miso_pin = "4"]
      [/#if]
      [#if item[1]?contains("PB4")]
        [#assign miso_port = "B"]
        [#assign miso_pin = "4"]
      [/#if]
      [#if item[1]?contains("PA10")]
        [#assign miso_port = "A"]
        [#assign miso_pin = "10"]
      [/#if]
      [#if item[1]?contains("PD5")]
        [#assign mosi_port = "D"]
        [#assign mosi_pin = "5"]
      [/#if]
      [#if item[1]?contains("PB8")]
        [#assign mosi_port = "B"]
        [#assign mosi_pin = "8"]
      [/#if]
      [#if item[1]?contains("PC3")]
  ...
  else
  {
    ...
    [#default]
      [#list  item[1] as pin]
        [#if pin?starts_with("P")]
          [#if pin?length?contains("3")]
	          [#assign d = pin[2]]
	        [#elseif pin?length?contains("4")]
	          [#assign d = pin[2] + pin[3]]
	        [#else]
	          [#if pin[3]?matches("([0-9])")]
	            [#assign d = pin[2] + pin[3]]
		        [#else]
		          [#assign d = pin[2]]
	          [/#if]
          [/#if]
  LL_GPIO_SetPinMode(GPIO${pin[1]}, LL_GPIO_PIN_${d}, LL_GPIO_MODE_OUTPUT);
        [/#if]
      [/#list]
  [/#switch]
[/#list]
...
[#list  MappedPins as item]
  ...
  [/#list]
  [#if x>0]
    [#if !item[0]?contains("RCC") && !item[0]?contains("I2C") && !item[0]?contains("SAI") && !item[0]?contains("DEBUG")]
    /* ${item[0]} */
    [/#if]
  [/#if]
  [#assign x = 0]
        ...
        [#if pin?starts_with("P")]
          [#if pin?length?contains("3")]
	        ...
	        [#else]
	          ...
	          [/#if]
          [/#if]
    LL_GPIO_SetPinMode(GPIO${pin[1]}, LL_GPIO_PIN_${d}, LL_GPIO_MODE_ALTERNATE);
        [/#if]
      [/#list]
  [/#if]
[/#list]
