...
[#if myHash["SNVMA_NVM_NUMBER"]?number != 0]
    ...
    [/#list]
[/#if]
[#-- On WBA2 Flash sector two times smaller, so needed to double needed number --]
[#if DIE=="DIE4B2"]
     [#assign SNVMA_NUMBER_OF_SECTOR_NEEDED = SNVMA_NUMBER_OF_SECTOR_NEEDED * 2]
[/#if]
...
#include "hw_if.h"
[/#if]
[#if ((myHash["BLE_MODE_HOST_SKELETON"] == "Enabled") || (myHash["BLE_MODE_PERIPHERAL_CENTRAL"] == "Enabled") || (myHash["BLE_MODE_CENTRAL"] == "Enabled") || (myHash["BLE_MODE_PERIPHERAL"] == "Enabled") || (myHash["BLE_MODE_SIMPLEST_BLE"] == "Enabled"))]
#include "ble_defs.h"
[/#if]
 ...
 */
#define CFG_BD_ADDRESS_DEVICE             (${myHash["CFG_BD_ADDRESS_DEVICE"]})
[#if ((myHash["CFG_STATIC_RANDOM_ADDRESS"]?number == 1) && (myHash["CFG_BD_ADDRESS_DEVICE"] == "GAP_STATIC_RANDOM_ADDR"))]
/**
 ...
 */
[#if (myHash["CFG_PRIVACY"] == "PRIVACY_ENABLED")]
#define CFG_BD_ADDRESS_TYPE               (${myHash["CFG_BD_ADDRESS_TYPE"]})
[#else]
#define CFG_BD_ADDRESS_TYPE               (${myHash["CFG_BD_ADDRESS_DEVICE"]})
[/#if]
[/#if]

[#if (myHash["BLE_MODE_PERIPHERAL"] == "Enabled")]
#define ADV_INTERVAL_MIN                  (${myHash["ADV_INTERVAL_MIN"]})
#define ADV_INTERVAL_MAX                  (${myHash["ADV_INTERVAL_MAX"]})
#define ADV_LP_INTERVAL_MIN               (${myHash["ADV_LP_INTERVAL_MIN"]})
#define ADV_LP_INTERVAL_MAX               (${myHash["ADV_LP_INTERVAL_MAX"]})
 ...
 */
#define CFG_BLE_EATT_BEARER_PER_LINK  (${myHash["CFG_BLE_EATT_BEARER_PER_LINK"]})
 ...
 */
#define CFG_BLE_HOST_EVENT_BUF_SIZE   (${myHash["CFG_BLE_HOST_EVENT_BUF_SIZE"]})

/**
 * Size of the RAM buffer allocated for the extension of BLE host commands
 */
#define CFG_BLE_EXTRA_DATA_BUF_SIZE (${myHash["CFG_BLE_EXTRA_DATA_BUF_SIZE"]})
[/#if]

/**
 * Size of the RAM buffer allocated for long write commands, can be 0 or 256.
 */
#define CFG_BLE_LONG_WRITE_DATA_BUF_SIZE (${myHash["CFG_BLE_LONG_WRITE_DATA_BUF_SIZE"]})
 ...
 ******************************************************************************/
#define CFG_LPM_LEVEL               (${myHash["CFG_LPM_LEVEL"]}U)

#define CFG_LPM_STOP1_SUPPORTED     (${myHash["CFG_LPM_STOP1_SUPPORTED"]}U)
[#if (myHash["HW_STOP2_SUPPORTED"]?number == 1)]
#define CFG_LPM_STOP2_SUPPORTED     (${myHash["CFG_LPM_STOP2_SUPPORTED"]}U)
[/#if]
#define CFG_LPM_STANDBY_SUPPORTED   (${myHash["CFG_LPM_STANDBY_SUPPORTED"]}U)
 ...
 */
#define CFG_LPM_WAKEUP_TIME_PROFILING (${myHash["CFG_LPM_WAKEUP_TIME_PROFILING"]}U)
 ...
 */
#define CFG_LPM_STDBY_WAKEUP_TIME (${myHash["CFG_LPM_STDBY_WAKEUP_TIME"]}U)
...
[#if (myHash["BLE_MODE_TRANSPARENT_UART"] == "Enabled") && (myHash["CFG_BLE_TM_PROTOCOL"] == "UART")]
  CFG_LPM_APP_BLE,
[/#if]
[#if (myHash["THREAD_SKELETON"] == "Enabled") || (myHash["THREAD"] == "Enabled")]
  CFG_LPM_APP_THREAD,
  CFG_LPM_PKA,
[/#if]
...
[#if (myHash["CFG_LOG_SUPPORTED"]?number != 0)]
...
#define CFG_LOG_TRACE_FIFO_SIZE     (${myHash["CFG_LOG_TRACE_FIFO_SIZE"]}U)
#define CFG_LOG_TRACE_BUF_SIZE      (${myHash["CFG_LOG_TRACE_BUF_SIZE"]}U)
 ...
 ******************************************************************************/
#define OT_CLI_USE                  (${myHash["CFG_CLI_SUPPORTED"]}U)
extern UART_HandleTypeDef           h${myHashCli["IpInstance"+i]?lower_case?replace("s","")};
...
[#if (myHash["ZGB_PERSISTENCE"] != "NONE")]
...
[#if (myHash["FREERTOS_STATUS"]?number == 1) || ((myHash["THREADX_STATUS"]?number == 1) && (myHash["BLE"] == "Enabled") || (myHash["BLE_MODE_SKELETON"] == "Enabled") || (myHash["BLE_MODE_HOST_SKELETON"] == "Enabled"))]
/******************************************************************************
 * RTOS configuration
 ******************************************************************************/
...
#define CFG_RT_DEBUG_DTB                    (${myHash["CFG_RT_DEBUG_DTB"]})
...
[#if ((myHash["CFG_SCM_SUPPORTED"]?number == 1) && (myHash["CFG_SCM_TIMER"] == "TIM17"))]
#define CFG_SCM_USE_TIM17                   (1)
[/#if]
[/#if]
...
#define CFG_HW_RNG_POOL_THRESHOLD           (${myHash["CFG_HW_RNG_POOL_THRESHOLD"]})
