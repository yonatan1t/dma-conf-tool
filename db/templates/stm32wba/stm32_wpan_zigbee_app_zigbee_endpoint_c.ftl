...
[#assign stackSmartEnergy = "false"]
[#assign PersistenceType = myHash["ZGB_PERSISTENCE"]]
        ...
        [#list SWIP.defines as configParameter]
            ...
            [#if configParameter.name?starts_with("ZCL_")]
                [#if configParameter.value != "NONE" && configParameter.value != "valueNotSetted"]
                    ...
                    [/#if]
                    [#if clusterName == "door_lock" && configParameter.value == "SERVER" ]
                      [#assign DoorLockServer = "true" ]
                    [#else]
                      [#assign DoorLockServer = "false" ]
                    [/#if]
                [/#if]
            [#else]
                ...
                [#else]
                    ...
                    [#else]
                        ...
                        [#-- [#else]
                            ...
                            [/#if] --]
                        [/#if]
                        [#if configParameter.name?contains("_PROFILE_ID") && configParameter.value?contains("ZCL_PROFILE_SMART_ENERGY")]
                            [#assign stackSmartEnergy = "true"]
                        [/#if]
                    [/#if]
                [/#if]
            [/#if]
    ...
    [#list clusterValue as seqElement]
        ...
        [#if (clusterRole == "server" && allocSpecificParamServer != "") || (clusterRole == "client" && allocSpecificParamClient != "")]
            ...
            [#else]
                ...
                [#lt]  if ( ZbZclClusterEndpointRegister( stZigbeeAppInfo.${clusterNameForCBName}${clusterRole?cap_first}${endpointName} ) == false )
                [#lt]  {
                [#lt]    LOG_ERROR_APP( "Error during ${clusterNameForCBName} ${clusterRole?cap_first} Endpoint Register." );
                [#lt]  }
            [/#if]
...
[#if DoorLockServer == "true" ]
    [#lt]#define MAX_PIN_NB_USERS                  5u
...
[#if zigbeeGenericParamHash_GP["ZGB_TCSO_TYPE"] == "SERVER" ]
...
[/#if]
[#if zigbeeGenericParamHash_GP["ZGB_TCSO_TYPE"] == "CLIENT" ]
