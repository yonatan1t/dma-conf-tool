...
[#assign UUID_TYPE = "UUID_TYPE_128"]
  ...
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_service command: ${SERVICE_SHORT_NAME}, error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_service command: ${SERVICE_SHORT_NAME?capitalize}SvcHdle = 0x%04X\n",${SERVICE_SHORT_NAME_UpperCase}_Context.${SERVICE_SHORT_NAME?capitalize}SvcHdle);
  }
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_BLE("  Fail   : aci_gatt_add_char command   : [@characteristicShortName characteristic/], error code: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_BLE("  Success: aci_gatt_add_char command   : [@characteristicShortNameCapitalized characteristic/]CharHdle = 0x%04X\n",${SERVICE_SHORT_NAME_UpperCase}_Context.[@characteristicShortNameCapitalized characteristic/]CharHdle);
  }
...
tBleStatus ${SERVICE_SHORT_NAME_UpperCase}_UpdateValue(${SERVICE_SHORT_NAME_UpperCase}_CharOpcode_t CharOpcode, ${SERVICE_SHORT_NAME_UpperCase}_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    ...
    case [@customServChar characteristic/]:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_BLE("  Fail   : aci_gatt_update_char_value [@characteristicShortName characteristic/] command, error code: 0x%02X\n", ret);
      }
      else
      {
        LOG_INFO_BLE("  Success: aci_gatt_update_char_value [@characteristicShortName characteristic/] command\n");
      }
...
tBleStatus ${SERVICE_SHORT_NAME_UpperCase}_UpdateValue_Ext(${SERVICE_SHORT_NAME_UpperCase}_CharOpcode_t CharOpcode, ${SERVICE_SHORT_NAME_UpperCase}_Data_t *pData, uint16_t EATT_Channel)
{
  ...
  switch(CharOpcode)
  {
    ...
    case [@customServChar characteristic/]:
      ...
      if (ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_BLE("  Fail   : aci_gatt_update_char_value_ext [@characteristicShortName characteristic/] command, error code: 0x%02X\n", ret);
      }
      else
      {
        LOG_INFO_BLE("  Success: aci_gatt_update_char_value_ext [@characteristicShortName characteristic/] command\n");
      }
