...
static TX_SEMAPHORE       PersistenceSemaphore;
[/#if]
[#if myHash["FREERTOS_STATUS"]?number == 1 ]
static osSemaphoreId_t    PersistenceSemaphore;
[/#if]
...
[#if myHash["THREADX_STATUS"]?number == 1 ]
  tx_semaphore_put( &PersistenceSemaphore );
[/#if]
[#if myHash["FREERTOS_STATUS"]?number == 1 ]
  osSemaphoreRelease( PersistenceSemaphore );
[/#if]
...
[/#if]
  ...
  {
[#if myHash["THREADX_STATUS"]?number == 1 ]
    tx_semaphore_get( &PersistenceSemaphore, TX_WAIT_FOREVER );
[/#if]
[#if myHash["FREERTOS_STATUS"]?number == 1 ]
...
[#if myHash["THREADX_STATUS"]?number == 1 ]
  /* Create the Persistence Task Semaphore */
  if ( tx_semaphore_create( &PersistenceSemaphore, "PersistSem", 0 ) != TX_SUCCESS )
  {
    LOG_ERROR_APP( "Persistence Semaphore creation FAILED" );
    eStatus = HAL_ERROR;
  }
...
[#if myHash["FREERTOS_STATUS"]?number == 1 ]
  /* Create the Persistence Task Semaphore */
  PersistenceSemaphore = osSemaphoreNew( 1U, 0U, NULL );
  if ( PersistenceSemaphore == NULL )
  {
    LOG_ERROR_APP( "Persistence Semaphore creation FAILED" );
    eStatus = HAL_ERROR;
  }
