...
static TX_THREAD        ClientDiscoverTaskHandle;
static TX_SEMAPHORE     ClientDiscoverSemaphore;
[/#if]
...
[#if myHash["THREADX_STATUS"]?number == 1 ]
  ...
  TXstatus = tx_byte_allocate(pBytePool, (void **)&pStack, TASK_STACK_SIZE_CLIENT_DISCOVER, TX_NO_WAIT);

  if( TXstatus == TX_SUCCESS )
  {
    TXstatus = tx_thread_create(&ClientDiscoverTaskHandle, "Client Discover  Task", client_discover_all_Entry, 0,
...
[#if (myHash["BLE_OPTIONS_GATT_CACHING"] == "BLE_OPTIONS_GATT_CACHING")]
      ...
      case PROC_GATT_ENABLE_ROBUST_CACHING:
      {
        ...
        if (a_ClientContext[index].ClientSupportedFeatureCharStartHdl != 0x0000)
        {
          ...
          {
            gatt_cmd_resp_wait();
            LOG_INFO_APP(" ClientSupportedFeatureCharValueHdl =0x%04X\n",a_ClientContext[index].ClientSupportedFeatureCharValueHdl);
          }
          else
          {
            LOG_INFO_APP(" ClientSupportedFeatureCharValueHdl write Failed, status =0x%02X\n\n", bleStatus);
          }
        }
...
void client_discover_all_Entry(unsigned long thread_input)
{
  ...
  while(1)
  {
    tx_semaphore_get(&ClientDiscoverSemaphore, TX_WAIT_FOREVER);
