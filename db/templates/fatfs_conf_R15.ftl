...
#define hUSB_Host ${definition.value}
      ...
      [/#if]
      [#if definition.name=="FF_USE_FIND"]                   [#-- New in R0.11 --]
      ...
      [/#if]
      [#if definition.name=="FF_USE_TRIM"]          [#-- Renamed in R0.11 --]
          [#assign valueUseTrim = definition.value]
      [/#if]
      [#if definition.name=="FF_FS_NOFSINFO"]
          [#assign valueNoFsinfo = definition.value]
      [/#if]
      [#if definition.name=="FF_FS_NORTC"]           [#-- New in R0.11 --]
          [#assign valueNoRtc = definition.value]
      [/#if]
      [#if definition.name=="FF_NORTC_YEAR"]         [#-- New in R0.11 --]
          [#assign valueNoRtcYear = definition.value]
      [/#if]
      [#if definition.name=="FF_NORTC_MON"]          [#-- New in R0.11 --]
          [#assign valueNoRtcMonth = definition.value]
      [/#if]
      [#if definition.name=="FF_NORTC_MDAY"]         [#-- New in R0.11 --]
      ...
      [/#if]
      [#if definition.name=="FF_USE_EXPAND"]       [#-- New in R0.12 --]
          [#assign valueUseExpand = definition.value]
      [/#if]
      [#if definition.name=="FF_USE_CHMOD"]        [#-- New in R0.12 --]
          [#assign valueUseChmod = definition.value]
      [/#if]
      [#if definition.name=="FF_FS_EXFAT"]         [#-- New in R0.12 --]
...
#define	_USE_EXPAND		${valueUseExpand}
...
[#-- In R0.15, definitions that depend on FreeRTOS state (enabled/disabled) --]
[#if cmsisrtosInUse == 1]
#include <stdlib.h>
[/#if]
