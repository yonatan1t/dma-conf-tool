...
[#assign allModules = ["ADC","BSEC","CRC","CRYP","DCMI","DCMIPP","DMA2D","DTS","ETH","EXTI","FDCAN","GFXMMU","GFXTIM","HASH","HCD","I2C","I2S","I3C","ICACHE","IRDA","IWDG","JPEG","LPTIM","LTDC","MCE","MDF","MMC","NAND","NOR","PCD","PKA","PSSI","RAMCFG","RIF","RNG","RTC","SAI","SD","SDIO","SDRAM","SMARTCARD","SMBUS","SPDIFRX","SPI","SRAM","TIM","UART","USART","WWDG","XSPI","CACHEAXI","MDIOS","GPU2D"]]
...
#define HSE_VALUE              [#if hse_value??]${hse_value}[#else]48000000[/#if]UL /*!< Value of the External oscillator in Hz */
#endif /* HSE_VALUE */
...
#define LSE_STARTUP_TIMEOUT    [#if LSE_Timout??]${LSE_Timout}[#if LSE_Timout?matches("(0x[0-9]*[a-f]*[A-F]*)|([0-9]*)")]UL[/#if][#else]5000U[/#if]     /*!< Time out for LSE start up, in ms */
...
#define  TICK_INT_PRIORITY          [#if TICK_INT_PRIORITY??]${TICK_INT_PRIORITY}U[#else]((uint32_t)((1UL<<__NVIC_PRIO_BITS) - 1UL)[/#if]  /*!< tick interrupt priority [#if !TICK_INT_PRIORITY?? || TICK_INT_PRIORITY == "15"](lowest by default) [/#if]*/
