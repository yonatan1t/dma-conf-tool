...
[#if DIE == "DIE450" || DIE == "DIE483" || DIE == "DIE480"]
/*
 * Define JPEG_SWAP_RB 
 */
        [#assign JPEG_SWAP = peripheralParams.get(ip).get("JPEG_SWAP")]
        [@compress single_line=true]
        [#assign val  = JPEG_SWAP]
        [#if val == "RB"]
        [#lt]#define JPEG_SWAP_RB  1  /* 0 or 1
        [#else]
        [#lt]#define JPEG_SWAP_RB  0  /* 0 or 1
        [/#if]
        [#if val == "RB"]
                [#lt] */
        [#else]
