...
[#if clock??]
    ...
    [/#list]
[/#if]
[#if (FamilyName=="STM32H7RS" && (isSBS_Used?? || isSBS_XSPIM1_Used?? || isSBS_XSPIM2_Used?? || isSBS_IO_Used??))||(FamilyName=="STM32H5" && isSBS_Used?? && !isSBSLL??)]
#t__HAL_RCC_SBS_CLK_ENABLE();
[/#if]
[#if (FamilyName == "STM32U5" || (DIE=="DIE494" || DIE=="DIE495")) && ((instName?? && (instName?starts_with("USB"))) || USBUsed??)]
...
[/#if]
[/#if]
[#if FamilyName == "STM32U3" ||FamilyName == "STM32N6"]
[#list IPdatas as IP]
...
[/#if]
[#if isSBS_VDDIO22_Used??||isSBS_VDDIO1_Used??]
#t/* Configure the compensation cell */
[/#if]
#n
[#if isSBS_VDDIO22_Used??]
[#if isSBSLL??]
#tLL_SBS_GetVddIOCellCompensationCode();
[#else]
#tHAL_SBS_VDDIOCompensationCodeSelect(SBS_VDDIO_CELL_CODE);
#tHAL_SBS_VDDIOCompensationCodeConfig(0U,0U);
[/#if]
[/#if]
#n
[#if isSBS_VDDIO1_Used??]
[#if isSBSLL??]
#tLL_SBS_GetVddCellCompensationCode();
[#else]
#tHAL_SBS_VDDCompensationCodeSelect(SBS_VDDIO_CELL_CODE);
#tHAL_SBS_VDDCompensationCodeConfig(0U,0U);
[/#if]
[/#if]
#n
[#if isSBS_VDDIO22_Used??||isSBS_VDDIO1_Used??]
...
[#if isSBS_VDDIO22_Used??]
[#if isSBSLL??]
#tLL_SBS_EnableVddIOCompensationCell();
[#else]
...
[#if isSBS_VDDIO1_Used??]
[#if isSBSLL??]
#tLL_SBS_EnableVddCompensationCell();
[#else]
...
#n
[#if isSBS_VDDIO22_Used??||isSBS_VDDIO1_Used??]
#t/* Compensation cell get ready flag status */
[/#if]
#n
[#if isSBS_VDDIO22_Used??]
[#if isSBSLL??]
#tLL_SBS_IsActiveFlag_VddIOCMPCR();
[#else]
#tHAL_SBS_GetVddIO2CompensationCellReadyFlag();
[/#if]
[/#if]
#n
[#if isSBS_VDDIO1_Used??]
[#if isSBSLL??]
#tLL_SBS_IsActiveFlag_VddCMPCR();
[#else]
#tHAL_SBS_GetVddIO1CompensationCellReadyFlag();
[/#if]
[/#if]
#n
    ...
    [#list configModel.clockEnableMacro as clkmacroList]
     [#if RESMGR_UTILITY?? && FamilyName=="STM32MP2"]
         ...
         [@generateResMgr ipName=V serviceType="Init" /] [#-- Enable ResMgr for RCC clock--] [/#if]
            [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
                                ...
                                [#if argument.value??]
                                    ...
                                    [/#if] [#-- if genericType=Array --]
                                      [#if RESMGR_UTILITY?? && FamilyName=="STM32MP2"]
                                          ...
                                          [#assign  seq=argValue]
                                          #n
                                          #tif (ResMgr_GPIO_Request(RESMGR_RESOURCE_RIF_${GPIO_PORT},RESMGR_GPIO_PIN(${seq})) != RESMGR_STATUS_ACCESS_OK)
                                          #t{
                                          #t#tError_Handler();
                                          #t}
                                          [/#if]  [/#if] [/#if]
...
#t#t__HAL_RCC_BKP_CLK_ENABLE();                    
				...
				[#if !((FamilyName=="STM32MP2" && ipName=="DCACHE") ||(FamilyName=="STM32WL3" && ipName=="MRSUBG_TIMER"))]
                [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
                ...
                [#list initService.clock?split(';') as clock][#-- debug ${ipvar.clkCommonResource}--]
                    ...
                    [#else]
					    [#if FamilyName=="STM32WL3" && ipName=="MRSUBG"]
							#t#tif (__HAL_RCC_MRSUBG_IS_CLK_DISABLED())#n#t#t{
								#t#t#t/* MRSUBG Peripheral reset */
								#t#t#t__HAL_RCC_MRSUBG_FORCE_RESET();
								#t#t#t__HAL_RCC_MRSUBG_RELEASE_RESET();
								#n#t#t#t/* Enable MRSUBG peripheral clock */
							    #t#t#t${clock?trim}[#if !clock?contains('(')]()[/#if];#n#t#t}#n
						[#elseif FamilyName=="STM32WL3" && ipName=="LPAWUR"]
							#t#tif (__HAL_RCC_LPAWUR_IS_CLK_DISABLED())#n#t#t{
								#t#t#t/* LPAWUR Peripheral reset */
								#t#t#t__HAL_RCC_LPAWUR_FORCE_RESET();
								#t#t#t__HAL_RCC_LPAWUR_RELEASE_RESET();
								#n#t#t#t/* Enable LPAWUR peripheral clock */
							    #t#t#t${clock?trim}[#if !clock?contains('(')]()[/#if];#n#t#t}#n
						[#elseif FamilyName=="STM32WB0" && ipName=="RADIO"]
                        ...
                        [#if FamilyName=="STM32WBA" && ipName=="PTA"]
                        #t#twhile(__HAL_RCC_PTACONV_IS_CLK_ENABLED() == 0U);
                        [/#if]
            ...
            [#if initService.clock!="none"]
              [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
                ...
                [#list initService.clock?split(';') as clock]     
                    [#if ipvar.clkCommonResource.entrySet()?contains(clock?trim) && DIE != "DIE4B0" && (DIE != "DIE485" || (DIE == "DIE485" && ipvar.ipName != "USB_OTG")) ]
 ...
 [@generateResMgr ipName=ipName serviceType="DeInit" /]  [#-- Disable ResMgr for RCC clock--] [/#if]
         ...
         [/#if]
         [#if FamilyName=="STM32WL3" && ipName=="MRSUBG"]
								#t#t__HAL_RCC_MRSUBG_FORCE_RESET();
								#t#t__HAL_RCC_MRSUBG_RELEASE_RESET();#n
         [/#if]
         [#if FamilyName=="STM32WL3" && ipName=="LPAWUR"]
								#t#t__HAL_RCC_LPAWUR_FORCE_RESET();
								#t#t__HAL_RCC_LPAWUR_RELEASE_RESET();#n
         [/#if]		 
    [/#if]
...
#n#t#t/* Enable VDDUSB */
	...
	#t#t{
	#t#t#tHAL_PWREx_EnableVddUSB();
    #t#t#tHAL_PWREx_EnableVdd11USB();
    #t#t#tHAL_PWREx_EnableUSBPWR();
    #t#t#tHAL_PWREx_EnableUSBBooster();
	#t#t}
	#t#telse
	#t#t{
    #t#t#t__HAL_RCC_PWR_CLK_ENABLE();
...
#n#t#t/* Disable VDDUSB */
	#t#tif(__HAL_RCC_PWR_IS_CLK_ENABLED())
	#t#t{
    #t#t#tHAL_PWREx_DisableUSBBooster();
    #t#t#tHAL_PWREx_DisableUSBPWR();
    #t#t#tHAL_PWREx_DisableVdd11USB();
	#t#t}
	#t#telse
	#t#t{
    #t#t#t__HAL_RCC_PWR_CLK_ENABLE();
...
[#if ipName?contains("USB") && (FamilyName=="STM32U3" || FamilyName=="STM32N6" || DIE = "DIE478" || DIE = "DIE484" || DIE=="DIE489" && DIE != "DIE474")]
[#if serviceType=="Init"]
#n#t#t/* Enable VDDUSB */
#t#tHAL_PWREx_EnableVddUSB();
[/#if]
[/#if]
[#if ipName?contains("USB") && FamilyName=="STM32N6"]
[#if serviceType=="DeInit"]
#n#t#t/* Disable VDDUSB */
#t#t#tHAL_PWREx_DisableVddUSB();
[/#if]
[/#if]
           ...
           [#if initService.clock??]
            [#if initService.clock!="none"]
              [#if FamilyName != "STM32MP2" || (ContextProject?? && !ContextProject?matches("A35TD_TZ(EN|DIS)"))]
               #t#t/* Peripheral clock enable */
                [#list initService.clock?split(';') as clock]
                    [#if ipvar.usedDriver?contains("HAL") &&  ipvar.clkCommonResource.entrySet()?contains(clock?trim) && DIE != "DIE4B0" && (DIE != "DIE485" || (DIE == "DIE485" && ipvar.ipName != "USB_OTG")) ]#t#t${clock?trim?replace("__","")?replace("_ENABLE","")}_ENABLED++;
                    #t#tif(${clock?trim?replace("__","")?replace("_ENABLE","")}_ENABLED==1){          
                        #t#t#t${clock?trim}[#if !clock?contains('(')]()[/#if];
                    [#if ipvar.usedDriver?contains("HAL") &&  ipvar.clkCommonResource.entrySet()?contains(clock?trim)]#t#t}[/#if]  
                    [#else]
            ...
            [#if ipName?contains("USB")]
                ...
                [#list initService.nvic as initVector]
                  [#if !initVector.vector?contains("WKUP") && !initVector.vector?contains("WakeUp") && initVector.codeInMspInit]
                   [#assign preemptionPriority = initVector.preemptionPriority]
                   [#assign subPriority = initVector.subPriority]
                   [#list nvic as vector]
                     [#if vector.isUsbHostDriver1() && mode?contains("HCD")]
                       [#assign preemptionPriority = vector.getInterruptPreemptionPriority()]
                       [#assign subPriority = vector.getInterruptSubPriority()]
                     [/#if]
                     [/#list]
                   [#if !initVector.vector?matches(".*(OTG_[H|F]S1_IRQn)")]
                    [#if DIE != "DIE501"]
                     #t#tHAL_NVIC_SetPriority(${initVector.vector}, ${preemptionPriority}, ${subPriority});
                    [#else]
...
[#-- End macro add service code --]
[#--------------DMA DFSDM --]
[#if ipvar.usedDriver?contains("HAL") && ipvar.clkCommonResource?? && DIE != "DIE4B0" && (DIE != "DIE485" || (DIE == "DIE485" && ipvar.ipName != "USB_OTG")) ]
...
[#if ((mode?contains("MRSubG") || mode?contains("MRSUBG_TIMER") || mode?contains("LPAWUR"))  && FamilyName=="STM32WL3") || mode?contains("PTA")]
#nvoid ${entry.key}(void)
{
[#elseif !mode?contains("TIM")||mode?contains("LPTIM")||mode?contains("HRTIM")||mode?contains("GFXTIM")]
    ...
    [#else] 
        ...
        [#else]
            [#if ipvar.instanceNbre > 1] [#-- IF of IP instances greater than 0--]
                 [#if !( ( words[0].contains("HASH") && DIE != "DIE503" ) || words[0].contains("SUBGHZ") || words[0].contains("MRSUBG") || words[0].contains("LPAWUR") || words[0].contains("PTA")) ]
...
[#else]
    [#if words[0]??]
[#if (words[0].contains("USB")) && (DIE == "DIE4B0" || DIE == "DIE485")]
#t#t/* USER CODE BEGIN ${words[0]}_${mode}_MspInit 0 */

#n#t#t/* USER CODE END ${words[0]}_${mode}_MspInit 0 */
[#elseif words[0] == "MRSUBG"]
#t#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 0 */

#n#t#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 0 */
[#else]
#t#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspInit 0 */
...
[#elseif words[0] == "MRSUBG"]
#t#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 1 */

#n#t#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspInit 1 */
[#else]
#t#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspInit 1 */
...
[#if !words[0].contains("HASH") &&  !words[0].contains("SUBGHZ") && !words[0].contains("MRSUBG") && !words[0].contains("LPAWUR") && !words[0].contains("PTA")]
#t}
[/#if]
[/#if]
[/#if]
...
[/#if]
[#if !( ( words[0].contains("HASH") && DIE != "DIE503" ) || words[0].contains("SUBGHZ") ) ]
#t}
[/#if]
  [#assign i = 0]
    [#list words as inst]
...
[#elseif words[0] == "MRSUBG"]
#t#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 0 */

#n#t#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 0 */
[#else]
#t#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspDeInit 0 */
...
[#elseif words[0] == "MRSUBG"]
#t#t/* USER CODE BEGIN ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 1 */

#n#t#t/* USER CODE END ${words[0]?replace("MRSUBG","MRSubG")}_MspDeInit 1 */
[#else]
#t#t/* USER CODE BEGIN ${words[0]?replace("I2S","SPI")}_MspDeInit 1 */
